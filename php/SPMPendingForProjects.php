...
//  I have 214 results
            ...
            for ( $i=0; $i<$QUERY->num_rows; $i++ ){
                ...
                $QUERY2     =   $LINK -> prepare("SELECT spm.id, spm.fechaRecepcion, spmProducto.id FROM spm INNER JOIN spmProducto ON spm.id = spmProducto.idSpm WHERE spmProducto.cantidadPendiente > 0 AND spm.estado = 'PENDIENTE' AND spmProducto.idPartida = ?");
                $QUERY2     ->  bind_param("i", $stageId);
                $QUERY2     ->  execute();
                $QUERY2     ->  store_result();
                $QUERY2     ->  bind_result($spmId, $spmDate, $spmProductoId);
